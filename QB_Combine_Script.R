...
#save to desktop --> easy to bring into Python --> called "qb_combine_dataset" on Github
write.csv(qb_combine,"C:\\Users\\drez_\\Desktop\\qb_combine_dataset.csv", row.names = TRUE)



#getting all combine data & metrics

#the all-time QB metrics that we want
qbs_combine =
  combine_dataset %>%
...
#bring in 2022 combine data from github --> "qb_data_online"
qb_2022_data = read.csv('https://github.com/ddale23/k_means_combine/blob/main/qb_data_online.csv')
...
#load in 2022 qb height & weight data set --> "qb_2022_combine"
...
all_qbs = read.csv('https://github.com/ddale23/k_means_combine/blob/main/all_qbs_clustered.csv')

#want to create some cluster names --> need to change cluster column to character type
...
#create  cluster viz with names on clusters
all_qbs %>%
...
#data set prep for table with 2022 prospects
table_data =
  qb_athletic %>%
...
#creating table with 2022 QB Prospects
table_data %>%
  ...
  tab_spanner(label = "Combine Metrics",
              columns = c(forty_percentile, vertical_percentile,
                          ...
                          shuttle_percentile)) %>%
  tab_spanner(label = "Combine Score",
              columns = avg_perc) %>%
  cols_width(
    player_name ~ 150,
    cluster_name ~150,
  ...
  data_color (
    ...
    colors = scales::col_factor(
      palette = c(
        "#44b348", "#1460e3", "#ff80e8", "#f76239", "#cc911b"), levels = c(
  ...
  fmt_percent(c(avg_perc, forty_percentile, vertical_percentile, broad_jump_percentile, cone_percentile,
                shuttle_percentile), decimals = 0) %>%
  tab_style(cell_text(weight = "bold"),
            locations = cells_title(
            ))


#code to get QBs within a range and then put data in a pretty table
qb_athletic %>%
  filter(draft_year >= 2010, avg_perc <= .93, avg_perc >= .73)  %>%
  select(player_name, draft_year, cluster_name, forty_percentile, vertical_percentile,
         broad_jump_percentile, cone_percentile, shuttle_percentile, avg_perc
  ) %>%
  arrange(-avg_perc) %>%
  gt(rowname_col = "player_name") %>%
  tab_header(
    title = "QBs within .10 Average Percentile from Perry",
    subtitle =  "") %>%
  cols_label(cluster_name = "Cluster",
...
#building a correlation data set to later compare draft year to average combine percentile 
correlation = 
  qb_athletic %>%
  filter(!is.na(avg_perc)) 

#getting correlation
cor(correlation$draft_year, correlation$avg_perc)


#graph showing Draft Year Compared to Average Percentile
correlation %>%
  ggplot(aes(draft_year, avg_perc)) + geom_point() +
  geom_smooth(method = "lm", se = F, color = "red") + 
  theme_bw() + 
  labs(x = "Draft Year",
       y = "Average Perctile",
       title = "Comparing Draft Year to Average Percentile for QBs",
       subtitle = "Correlation = 0.028") +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle =  element_text(hjust = 0.5))
